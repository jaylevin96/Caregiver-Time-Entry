...
/** Compact duration for payment copy, e.g. "4 hours 30 min". */
export function formatHoursCopy(hours: number): string {
  if (hours <= 0) return '0 hours';
  ...
  const hourPart =
    wholeHours === 0
      ...
      : wholeHours === 1
        ? '1 hour '
        : `${wholeHours} hours `;
  const minutePart = minutes === 1 ? '1 min' : `${minutes} min`;

  return `${hourPart}${minutePart}`.trim();
}

/** Payment copy date, e.g. "7-1-2026". */
export function formatPayCopyDate(dateStr: string): string {
  const { y, m, d } = parseDateOnly(dateStr);
  return `${m}-${d}-${y}`;
}

export function entryHours(hours: number | null | undefined): number {
  return hours ?? 0;
}
